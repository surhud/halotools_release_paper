...
\title[Presenting Halotools v0.2]
{
...
\begin{lstlisting}[float=*t, label=some-code,caption = Loading a cached halo catalog into memory \label{code:cachedsim}]
...
We thank all Halotools users who have patiently helped improve the package by providing feedback and critiques from the project's alpha-stages to the present. We thank the Astropy developers for the package-template, which has been critical to the entire process of Halotools development. 
